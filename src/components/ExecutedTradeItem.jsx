function ExecutedTradeItem({ createdAt, executedAt, limitPrice, orderType, price, quantity, side, status, ticker, totalPrice, type}) {
  ...
  return (
    <div className="bg-neutral-500 p-3 rounded text-white flex flex-col gap-1">
      ...
      <div>
        <h1 className="text-neutral-300 text-lg font-bold">Status: {status}</h1>
      </div>      
      <div>
      ...
      {status !== 'FAILED' && (
        <>
            <div>
                <h1 className="text-neutral-300 text-lg font-bold">Price: ${price?.toFixed(2)}</h1>
            </div>
            <div>
                <h1 className="text-neutral-300 text-lg font-bold">Price: ${totalPrice?.toFixed(2)}</h1>
            </div>
        </>
      )}
      ...
      <div>
        <h1 className="text-neutral-300 text-lg font-bold">Executed: {formatDate(executedAt)}</h1>
      </div>
    </div>
  )
}

export default ExecutedTradeItem